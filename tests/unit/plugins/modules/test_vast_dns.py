"""Unit tests for stevefulme1.vastdata.vast_dns module."""
...
class TestCreate:
    ...
    def test_create_resource(self, mock_get_client):
        ...
        mock_client.post.return_value = {
            "id": 1,
            "name": "test-dns",
            "vip_pool_id": 1, "domain_suffix": 'example.com', "vms_preferred": '8.8.8.8',
        }
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_dns import VastDns
        obj = VastDns(module)
        result = obj.create_resource()

        mock_client.post.assert_called_once()
        assert result["id"] == 1
        assert result["name"] == "test-dns"
...
class TestDelete:
    ...
    def test_delete_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_dns import VastDns
        obj = VastDns(module)
        obj.delete_resource({"id": 42, "name": "test-dns"})

        mock_client.delete.assert_called_once_with("/api/dns/42/")


class TestUpdate:
    """Test vast_dns update."""
    ...
    def test_update_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_dns import VastDns
        obj = VastDns(module)
        result = obj.update_resource({"id": 1, "name": "test-dns"})

        mock_client.patch.assert_called_once()
        assert result["name"] == "test-dns-updated"
...
class TestErrorHandling:
    ...
    def test_not_found_on_delete(self, mock_get_client):
        ...
        try:
            obj.delete_resource({"id": 999, "name": "test-dns"})
        except Exception as e:
            assert "404" in str(e)
