"""Unit tests for stevefulme1.vastdata.vast_nfs_export module."""
...
class TestCreate:
    ...
    def test_create_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_nfs_export import VastNfsExport
        obj = VastNfsExport(module)
        result = obj.create_resource()

        mock_client.post.assert_called_once()
        assert result["id"] == 1
        assert result["name"] == "nfs-data"
...
class TestDelete:
    ...
    def test_delete_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_nfs_export import VastNfsExport
        obj = VastNfsExport(module)
        obj.delete_resource({"id": 42, "name": "nfs-data"})

        mock_client.delete.assert_called_once_with("/api/nfsexports/42/")


class TestUpdate:
    """Test vast_nfs_export update."""
    ...
    def test_update_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_nfs_export import VastNfsExport
        obj = VastNfsExport(module)
        result = obj.update_resource({"id": 1, "name": "nfs-data"})

        mock_client.patch.assert_called_once()
        assert result["name"] == "nfs-data-updated"
...
class TestErrorHandling:
    ...
    def test_not_found_on_delete(self, mock_get_client):
        ...
        try:
            obj.delete_resource({"id": 999, "name": "nfs-data"})
        except Exception as e:
            assert "404" in str(e)
