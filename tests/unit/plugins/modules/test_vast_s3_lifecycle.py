"""Unit tests for stevefulme1.vastdata.vast_s3_lifecycle module."""
...
class TestCreate:
    """Test vast_s3_lifecycle creation."""
    ...
    def test_create_resource(self, mock_get_client):
        ...
        mock_client.post.return_value = {
            "id": 1,
            "name": "expire-30d",
            "prefix": 'logs/', "expiration_days": 30, "view_id": 1,
        }
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_s3_lifecycle import VastS3Lifecycle
        obj = VastS3Lifecycle(module)
        result = obj.create_resource()

        mock_client.post.assert_called_once()
        assert result["id"] == 1
        assert result["name"] == "expire-30d"
...
class TestDelete:
    """Test vast_s3_lifecycle deletion."""
    ...
    def test_delete_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_s3_lifecycle import VastS3Lifecycle
        obj = VastS3Lifecycle(module)
        obj.delete_resource({"id": 42, "name": "expire-30d"})

        mock_client.delete.assert_called_once_with("/api/s3lifecyclerules/42/")


class TestUpdate:
    """Test vast_s3_lifecycle update."""
    ...
    def test_update_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_s3_lifecycle import VastS3Lifecycle
        obj = VastS3Lifecycle(module)
        result = obj.update_resource({"id": 1, "name": "expire-30d"})

        mock_client.patch.assert_called_once()
        assert result["name"] == "expire-30d-updated"
...
class TestErrorHandling:
    ...
    def test_not_found_on_delete(self, mock_get_client):
        ...
        try:
            obj.delete_resource({"id": 999, "name": "expire-30d"})
        except Exception as e:
            assert "404" in str(e)
