"""Unit tests for stevefulme1.vastdata.vast_active_directory module."""
...
class TestCreate:
    """Test vast_active_directory creation."""
    ...
    def test_create_resource(self, mock_get_client):
        ...
        mock_client.post.return_value = {
            "id": 1,
            "machine_account_name": "VAST$",
            "domain_name": 'corp.example.com', "preferred_dc_list": ['dc1.corp.example.com'],
        }
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_active_directory import VastActiveDirectory
        obj = VastActiveDirectory(module)
        result = obj.create_resource()

        mock_client.post.assert_called_once()
        assert result["id"] == 1
        assert result["machine_account_name"] == "VAST$"
...
class TestDelete:
    """Test vast_active_directory deletion."""
    ...
    def test_delete_resource(self, mock_get_client):
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_active_directory import VastActiveDirectory
        obj = VastActiveDirectory(module)
        obj.delete_resource({"id": 42, "machine_account_name": "VAST$"})

        mock_client.delete.assert_called_once_with("/api/activedirectory/42/")


class TestUpdate:
    """Test vast_active_directory update."""
    ...
    def test_update_resource(self, mock_get_client):
        ...
        mock_client.patch.return_value = {
            "id": 1,
            "machine_account_name": "VAST$-updated",
        }
        ...
        from ansible_collections.stevefulme1.vastdata.plugins.modules.vast_active_directory import VastActiveDirectory
        obj = VastActiveDirectory(module)
        result = obj.update_resource({"id": 1, "machine_account_name": "VAST$"})

        mock_client.patch.assert_called_once()
        assert result["machine_account_name"] == "VAST$-updated"
...
class TestErrorHandling:
    ...
    def test_not_found_on_delete(self, mock_get_client):
        ...
        try:
            obj.delete_resource({"id": 999, "machine_account_name": "VAST$"})
        except Exception as e:
            assert "404" in str(e)
